...
Below is not only an investigation into C4.5 but also, a comparison of two variations. Initinally all available dimensions will be used for training.
...
The J48 function is extremely quick with the dataset, training takes less than a second, which on it's on is not necessarily worthy of note but certainly more interesting when compared to the speed of using the caret train method with a "J48" method arguemtn value.

We can look at the complexity of the tree by looking at the dimensionality, where the length is effectively the tree size, width is the number of leaves, or terminal nodes and the depth is effectivly the number of conditional branch layers.
...
The complexity of this tree is sufficiently complex to render a graphical representation useless. In fact the tree is complext it's time intensive as well as being of no value.
...
Even though the tree is able to be rendered it's not easy to get anything meaning ful out of this. Perhaps the most pertinent point is that the not only could the cliasification levels benefit from being simplified but using fewer dimensions for observation would also force a simpler set of conditional branching.

###Comparing the two C4.5/J48 methods

Below follows the summaires from both methods, for examniation: 
```{r}
dt_sum.c4_5_j48_h
dt_sum.c4_5_h2
```
...
Despite the values being present in the summaries, to clafiry understanding, they are repeated below:
...
The J48 function has a significantly higher accuracy compared to the train function J48 call but at this stage it's hard to be confident this is a good thing; given the difference in tree complexity, it could well be that the J48 function suffers from massive overfitting, while the train call has done some excessive pruning which has not noly accounted for the extra time for the function to complete but also the diminished accuracy. To understand things further it's really necessary to test the trees against the validation subset.
...
```
It would appear that despite the J48 function derived model claiming a higher accuracy, the resulting confustion matric data suggests otherwise. Neither 23.6% or 20.9 is particulary encouraging, however for the improvement in speed, the loss of about 12.% in accuracy (approximately 3% from 24%), might be reasonable is the overall accuracy can be improved. To investigate this further, the next steps are to look at reducing the number of attrbutes observed based on the earlier analysis of data, and  training the models to achieve simpler classification goals.

###C4.5 with refined dataset

To attempt attainment of high accuracy, it's worth looking to run the very same decision tree functions against a streamline classification aim, with a more targetted formula. Using raw rings and the classification levels amounted to nearly 30 possible outomces, so bringing that down do a concrete tiered factor of 3 age groups has to make things not only more perormant but it's easier to be more accurate when the intened classification type has a larger scope.
...
Interestingly the J48 function has produced an accurnacy rating that is only marginally better than the previous version while the train J48 function call seems to have improved significantly so that it's nearly on par with the J48 function result. Looking at the models after validation has happened will hopefully provide even more revealing findings.
...
The accuracy has improved markedly so it's safe to say that the combinatation of streamliining the formula and creating a simpler classification requirement hase improved things; this is the new baseline, now it's worth looking at any improvment that can be made through using the k-folds and leave-one-out cross-validation technues.
...
Unfortunately, for the perfomant J48 method, none of these options seems to allow for custom methods of training. At this point, J48 has to be disregarded due to inflexibilty despite such good runtime training speeds.

```{r dt-c4_5_j48-train-rkf_a}

##dt.c4_5_j48_kf_a <- J48(dt.formula, holdout_age_groups.training)
#dt.c4_5_j48_kf_a <- J48(dt.formula, holdout_age_groups.training, control = tempcv.train_control_8_5)
dt.c4_5_j48_kf_a <- train(dt.formula, method="J48", holdout_age_groups.training, tuneLength = 8, trControl = cv.train_control_8)
#J48(dt.formula, holdout_age_groups.training, control = tempcv.train_control_8_5)
```

```{r dt-c4_5_train-h_a_results}
dt_sum.c4_5_h2_a <- summary(dt.c4_5_j48_kf_a)
