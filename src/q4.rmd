...
```{r nb-h_a}
nb.h_a <- NaïveBayes(dt.formula, data=holdout_age_groups.training)
```
...
What's interesting about these Naïve Bayes plots is that they suggest the merit of using this techniques to select features that could be used when creating a formula for use with another model. Another aspect worth noting is how the height and diameter probability graphs are extremely similar, which could suggest redundancy, where another feature could have been used as a classifier variable.
